...
def gdisconnect():
    ...
    # if credentials.access_token != login_session['access_token']:
        # raise ValueError('access_tokens dont match')
...
def showRestaurants():
    owners = session.query(Restaurant.user_id).all()
...
def editRestaurant(restaurant_id):
    ...
    owner = editedRestaurant.user_id
    if 'username' not in login_session:
        return redirect('/login')
...
def editMenuItem(restaurant_id, menu_id):
    ...
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
    if 'username' not in login_session or login_session['username'] != resta:
        return redirect('/login')
    if request.method == 'POST':
        if request.form['name']:
